...
function begin_play(){
	...
	{
		alert('One or more of the player names is empty!');
		return;
	}
	...
	document.getElementById('turn_info').innerHTML = "X";
...
function play() {
	...
	{
		alert('Invalid move. Please only enter a cell number.');
		return;
	}

	if(document.getElementById(move.value).innerHTML != move.value)
	{
		alert('Invalid move. Please make a move in an unfilled cell.');
		return;
	}

	var cell = document.getElementById(move.value);
	var turn_disp = document.getElementById('turn_info');

	if(this.turn == 1)
	{
		cell.innerHTML = 'X';
		turn_disp.innerHTML = 'Turn for: O';
	}
	else
	{
		cell.innerHTML = 'O';
		turn_disp.innerHTML = 'Turn for: X';
	}
...
function validate()
{
	...
	for(i=0; i<7; i+=3)
	{
		head = document.getElementById(table_ids[i]).innerHTML;
		next = document.getElementById(table_ids[i+1]).innerHTML;
		last = document.getElementById(table_ids[i+2]).innerHTML;
	...
	{
		head = document.getElementById(table_ids[i]).innerHTML;
		next = document.getElementById(table_ids[i+3]).innerHTML;
		last = document.getElementById(table_ids[i+6]).innerHTML;
		console.log(head)
		if(head == next && head == last)
			win = head;
	}

	if(document.getElementById(table_ids[0]).innerHTML == document.getElementById(table_ids[4]).innerHTML && document.getElementById(table_ids[0]).innerHTML == document.getElementById(table_ids[8]).innerHTML)
		win = document.getElementById(table_ids[0]).innerHTML;

	if(document.getElementById(table_ids[2]).innerHTML == document.getElementById(table_ids[4]).innerHTML && document.getElementById(table_ids[2]).innerHTML == document.getElementById(table_ids[6]).innerHTML)
		win = document.getElementById(table_ids[2]).innerHTML;
	...
	if(full)
	{
		alert('Moves have been exhausted.');
		reset_play();
	}
}
